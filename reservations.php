...
<body>
                ...
                <ul class="navbar-nav ml-auto navbar-nav-scroll" style="--bs-scroll-height: 160px;">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu navbar-light bg-dark">
                            ...
                            <li><a class="dropdown-item active" href="reservations.php">Reserve Studio</a></li>
    ...
    <div>
        <section class="shop container">
            ...
            while ($row = mysqli_fetch_assoc($query)) { 
                ...
                $name = $row['name'];
                $rateh = $row['hourly_rate'];
                $imagePath = $row['imagepath'];
                ?>
                <div class="product-box mngPack">
                    <div class="popp"  data-item-id="<?php echo $package_id; ?>">
                        <img src="admin/<?php echo $imagePath ?>" alt="" class="product-img">
                        <h2 class="product-title"><?php echo $name ?></h2>
                    </div>
                        <span><input type="hidden" class="item-quantity" name="item-quantity" value="1"></span>
                        <span class="price">Rs. <?php echo $rateh ?> /h<br/></span>
    ...
    <!-- Instrument Addons -->
    <section class="shop container">
            <h2 class="section-title">Addon Instruments</h2>
            <div class="shop-content">
            ...
            $conn = mysqli_connect($host, $usernamedb, $password, $database);
            // Retrieve data from the "instruments" table
            $query = mysqli_query($conn, "SELECT instrument_id, name, type, brand, model, quantity, hourlyrate, description, imagepath FROM instruments");
            if (!$query) {
                die('Error executing query: ' . mysqli_error($conn));
            }         
            // Display each item
            while ($row = mysqli_fetch_assoc($query)) { 
                ...
                <div class="product-box">
                    <div class="pop"  data-item-id="<?php echo $instrument_id; ?>">
                        ...
                        <span class="price">Rs. <?php echo $rateh ?> /h</span>                    
                    <svg class="add-cart" xmlns="http://www.w3.org/2000/svg" height="1.3em" viewBox="0 0 448 512">
                        <style>svg{fill:#000000}</style>
                    <path d="M160 112c0-35.3 28.7-64 64-64s64 28.7 64 64v48H160V112zm-48 48H48c-26.5 0-48 21.5-48 48V416c0 53 43 96 96 96H352c53 0 96-43 96-96V208c0-26.5-21.5-48-48-48H336V112C336 50.1 285.9 0 224 0S112 50.1 112 112v48zm24 48a24 24 0 1 1 0 48 24 24 0 1 1 0-48zm152 24a24 24 0 1 1 48 0 24 24 0 1 1 -48 0z"/>
                    </svg>
        ...
        function showModal(event) {
            ...
            .then(itemDetails => {
                ...
                modalBodyR.innerHTML = `
                    ...
                    <p class="item-price pt-3">Rs. ${itemDetails.hourlyrate} /h</p>
...
<script>
        document.addEventListener("DOMContentLoaded", function () {
        // Get all elements with class "product-box"
        const productBoxes = document.querySelectorAll(".popp");
        ...
        function showModal(event) {
            ...
            .then(itemDetails => {
                ...
                modalBodyR.innerHTML = `
                    <h2 class="product-title">${itemDetails.name}</h2>
                    <p class="item-price pt-3">Rs. ${itemDetails.hourlyrate} /h</p>
                    <input type="hidden" class="item-quantity-value" name="item-quantity" value="1">
                    <p class="item-description">Description: <br>${itemDetails.package_constituents}</p>
                `;
